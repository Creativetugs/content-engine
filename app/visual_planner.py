...
Outputs: {outputs}
Company context: {(settings.brand_kit or {}).get('company_gist', '') if isinstance(settings.brand_kit, dict) else ''}
Visual notes: {(settings.brand_kit or {}).get('visual_style_notes', '') if isinstance(settings.brand_kit, dict) else ''}
...
- Prefer realistic industry/product imagery that matches this company — avoid generic cartoon clipart unless image_style is illustration
