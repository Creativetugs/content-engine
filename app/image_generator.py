...
IMAGE_CONCURRENCY = int(os.getenv("CE_IMAGE_CONCURRENCY", "1"))
...
def _prioritize_assets(
    ...
    limit = MAX_ASSETS if max_visuals is None else max(0, min(int(max_visuals), MAX_ASSETS))
    if limit <= 0:
        return []
    return sorted(assets, key=_role_sort_key)[:limit]
...
def prioritize_assets(
    ...
    """Public helper to trim a visual plan by role priority."""
    return _prioritize_assets(assets, max_visuals=max_visuals)
    ...
    for attempt in range(4):
        try:
            ...
            if "429" in last_error or "rate" in last_error.lower():
                wait = 2 ** attempt
                logger.info("Image rate limited, retry in %ss (%s)", wait, asset.get("id", "asset"))
                time.sleep(wait)
                continue
            return {**asset, "url": "", "status": "failed", "error": last_error}

    return {**asset, "url": "", "status": "failed", "error": last_error or "Rate limited"}
...
def generate_visual_assets(
    ...
    assets = _prioritize_assets(plan.get("assets", []), max_visuals=max_visuals)
    if not assets:
        return []
