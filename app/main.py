...
_cors_origins = os.getenv("CE_CORS_ORIGINS", "*").strip()
if _cors_origins:
    app.add_middleware(
        CORSMiddleware,
...
def _http_error_from_openai(exc: Exception) -> HTTPException | None:
    """Map OpenAI client errors to clear 502 responses for the WordPress plugin."""
    try:
    ...
    if isinstance(exc, AuthenticationError):
        return HTTPException(
            status_code=502,
            detail="OpenAI API key is invalid. Update OPENAI_API_KEY on Railway.",
        )
    if isinstance(exc, RateLimitError):
        ...
        if "insufficient_quota" in message or "quota" in message:
            return HTTPException(
                status_code=502,
                detail=(
                    "OpenAI account has no quota left. Add billing or credits at "
                    "platform.openai.com/account/billing, then retry."
                ),
            )
        return HTTPException(
            status_code=502,
            detail="OpenAI rate limit hit. Wait a minute and retry.",
        )
...
def health():
    ...
    try:
        ...
        payload["licensing_enabled"] = licensing_enabled()
    except Exception:
        payload["licensing_enabled"] = False

    return payload


def _license_key_from_request(request: Request, fallback: str = "") -> str:
    header = request.headers.get("x-ce-license") or request.headers.get("X-CE-License")
    if header:
...
def _license_guard(
    ...
    *,
    ...
    from app.licensing import license_check

    try:
        return license_check(
            license_key,
            increment=increment,
            need_visuals=need_visuals,
...
def _billable_visual_assets(assets) -> bool:
    from app.licensing import billable_visual_assets

    return billable_visual_assets(assets)


def _license_usage_for_visuals(license_key: str, assets, *, increment: bool) -> dict:
    from app.licensing import count_billable_assets

    image_count = count_billable_assets(assets)
    if increment and _billable_visual_assets(assets):
    ...
    return _license_guard(license_key, increment=False)
...
def admin_dashboard_page():
    """Creative Tugs operator dashboard — manage clients, limits, resets."""
...
def license_usage_endpoint(request: Request, license_key: str = ""):
    """Return monthly project usage for a license key (WordPress Dashboard)."""
    key = _license_key_from_request(request, license_key)
    return _license_guard(key, increment=False)
...
@app.post("/license/reset")
def license_reset_endpoint(request: Request, body: LicenseResetRequest):
    """Reset monthly usage for a client license (Creative Tugs admin only)."""
    from app.licensing import licensing_enabled, reset_usage

    _require_admin_secret(request)
    if not licensing_enabled():
        return reset_usage()

    key = _license_key_from_request(request, body.license_key)
    if body.all:
        return reset_usage(all_keys=True)
    if not key:
        raise HTTPException(status_code=400, detail="license_key is required unless all=true.")
    try:
        return reset_usage(key)
    except ValueError as exc:
...
def _build_response(
    ...
    # Cap transcript size returned to WordPress — large PDFs were blowing
    # AJAX/transient limits and causing the visuals step to never start.
    max_transcript_chars = int(os.getenv("CE_CLIENT_TRANSCRIPT_MAX", "40000"))
    client_transcript = transcript or ""
    if len(client_transcript) > max_transcript_chars:
        client_transcript = (
            client_transcript[:max_transcript_chars]
            + "\n\n[Transcript truncated for client transfer]"
        )
...
def _settings_from_wp_payload(wp_payload: dict) -> BrandSettings:
    ...
    return BrandSettings(
        ...
        image_style=brand_data.get("image_style", "mixed"),
        target_word_count=brand_data.get("target_word_count", 5500),
        website_url=brand_data.get("website_url", "") or "",
        brand_kit=kit,
    )


def _merge_license_brand_kit(settings: BrandSettings, license_key: str) -> BrandSettings:
    """Prefer request kit; fall back to remembered kit on the license."""
    if settings.brand_kit:
        return settings
    if not license_key:
        return settings
    try:
        from app.licensing import get_brand_kit

        kit = get_brand_kit(license_key)
    except Exception:
        return settings
    if not kit:
        return settings
    updates: dict = {"brand_kit": kit}
    if not settings.website_url and kit.get("source_url"):
        updates["website_url"] = kit.get("source_url", "")
    # Fill empty brand fields from kit when present.
    if settings.industry in ("", "General") and kit.get("industry"):
        updates["industry"] = kit["industry"]
    if settings.audience in ("", "General audience") and kit.get("audience"):
        updates["audience"] = kit["audience"]
    if settings.tone in ("", "Professional") and kit.get("tone"):
        updates["tone"] = kit["tone"]
    if kit.get("writing_style"):
        updates.setdefault("writing_style", kit["writing_style"])
...
def regenerate_visual(request: Request, payload: RegenerateVisualRequest):
    ...
    key = _license_key_from_request(request, payload.license_key)
    _license_guard(key, need_visuals=True)
...
def generate_visuals_batch(request: Request, body: GenerateVisualsRequest):
    ...
    try:
        ...
        settings = _merge_license_brand_kit(settings, key)
    ...
    try:
        run_visual_pipeline(content, settings, base_url, max_visuals=body.max_visuals)
    except ValueError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    except Exception as exc:
    ...
    response = _apply_visuals_to_wp_payload(wp_payload, content)
    assets = (content.get("visuals") or {}).get("assets") or []
    usage = _license_usage_for_visuals(key, assets, increment=True)
    if usage:
        response["license_usage"] = usage
    return response


@app.post("/visuals/plan")
def visuals_plan(request: Request, body: GenerateVisualsRequest):
    """Plan which images to generate (fast GPT call only)."""
    from app.image_generator import prioritize_assets
    from app.visual_planner import plan_visuals

    key = _license_key_from_request(request, body.license_key)
    _license_guard(key, need_visuals=True, image_count=body.max_visuals or 0)
    ...
    try:
        ...
        if body.max_visuals is not None:
            plan = {
                **plan,
                "assets": prioritize_assets(plan.get("assets", []), max_visuals=body.max_visuals),
            }
    except (ValidationError, ValueError) as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    except Exception as exc:
...
def brand_analyze(request: Request, body: BrandAnalyzeRequest):
    """Fetch a company website and return (optionally remember) a brand kit."""
    from app.brand_research import analyze_website
    from app.licensing import save_brand_kit

    key = _license_key_from_request(request, body.license_key)
    # Analyzing uses GPT text only — no visuals spend check beyond basic license.
    _license_guard(key, increment=False, need_visuals=False)

    try:
        kit = analyze_website(body.website_url)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:
        openai_err = _http_error_from_openai(exc)
        if openai_err:
            raise openai_err from exc
        raise HTTPException(status_code=502, detail=f"Brand analysis failed: {exc}") from exc

    remembered = False
    if body.remember and key:
        try:
            save_brand_kit(key, body.website_url.strip(), kit)
            remembered = True
        except ValueError as exc:
            # Still return kit even if license memory fails (e.g. licensing off).
            logger.warning("Could not remember brand kit: %s", exc)
    ...
    return {
        ...
        "industry": kit.get("industry") or "",
        "audience": kit.get("audience") or "",
        "writing_style": kit.get("writing_style") or "",
        "cta_style": kit.get("cta_style_hint") or "",
    }
...
def visuals_attach(request: Request, body: VisualAttachRequest):
    ...
    response = _apply_visuals_to_wp_payload(body.payload, content)
    usage = _license_usage_for_visuals(key, body.assets, increment=True)
    if usage:
        response["license_usage"] = usage
    return response
...
def regenerate_content(request: Request, payload: RegenerateContentRequest):
    ...
    key = _license_key_from_request(request, payload.license_key)
    _license_guard(key, need_visuals=payload.generate_visuals)
    ...
    result = _build_response("regenerate", transcript, structured_content, settings)
    from app.licensing import licensing_enabled

    if licensing_enabled():
        assets = (structured_content.get("visuals") or {}).get("assets") or []
        result["license_usage"] = _license_usage_for_visuals(
            key,
            assets,
            increment=bool(payload.generate_visuals),
        )
    return result
...
def generate_idea(request: Request, payload: GenerateIdeaRequest):
    ...
    key = _license_key_from_request(request, payload.license_key)
    _license_guard(key, increment=False, need_visuals=payload.generate_visuals)
...
async def repurpose(
    ...
    image_style: str = Form("mixed"),
    target_word_count: int = Form(5500),
    ...
    kit_data: dict = {}
    if brand_kit and brand_kit.strip():
        try:
            parsed_kit = json.loads(brand_kit)
            if isinstance(parsed_kit, dict):
                kit_data = parsed_kit
    ...
    client_license = _license_key_from_request(request, license_key)
    settings = _merge_license_brand_kit(settings, client_license)

    max_vis = None
    if max_visuals and str(max_visuals).strip().isdigit():
        max_vis = max(0, min(8, int(max_visuals)))
        if max_vis == 0:
            should_generate_visuals = False

    _license_guard(
        client_license,
        increment=False,
        need_visuals=should_generate_visuals,
        image_count=max_vis or 0,
    )
    ...
    if blog_input:
        ...
        from app.pdf_extract import extract_pdf_text, is_pdf_file

        if is_pdf_file(filename, getattr(file, "content_type", None)):
            t1 = time.perf_counter()
            pdf_data, pdf_error = extract_pdf_text(upload_path)
            if pdf_error or not pdf_data:
                raise HTTPException(status_code=400, detail=pdf_error or "Could not read PDF.")
            transcript = pdf_data["text"]
            source = {
                "type": "pdf",
                "filename": filename,
                "title": pdf_data.get("title") or filename,
                "page_count": pdf_data.get("page_count", 0),
                "transcript_source": "pdf_text",
            }
            logger.info("Extracted PDF text in %.1fs", time.perf_counter() - t1)
        else:
            t1 = time.perf_counter()
            try:
                from app.audio_prep import prepare_audio_for_whisper

                whisper_path = prepare_audio_for_whisper(upload_path)
                transcript = transcribe_audio(whisper_path)
            except ValueError as exc:
                raise HTTPException(status_code=400, detail=str(exc)) from exc
            except Exception as exc:
                detail = re.sub(r"\x1b\[[0-9;]*m", "", str(exc)).strip()
                if "maximum" in detail.lower() and "size" in detail.lower():
                    detail = (
                        "File too large for transcription (OpenAI Whisper limit is 25 MB). "
                        "Upload a shorter clip or use a YouTube link with captions."
                    )
                raise HTTPException(status_code=502, detail=detail or "Transcription failed.") from exc
            logger.info("Transcribed in %.1fs", time.perf_counter() - t1)
    else:
        raise HTTPException(
            status_code=400,
            detail="Provide a media file, PDF, blog URL, video URL, or written idea.",
        )
    ...
    if licensing_enabled():
        assets = (result.get("visuals") or {}).get("assets") or []
        result["license_usage"] = _license_usage_for_visuals(
            client_license,
            assets,
            increment=should_generate_visuals,
        )
    return result
